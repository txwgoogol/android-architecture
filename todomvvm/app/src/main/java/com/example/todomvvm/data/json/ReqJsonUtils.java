package com.example.todomvvm.data.json;
...
public class ReqJsonUtils {
    ...
    public static <T> T parseHttpResult(TypeInfo type, String jsonData) throws JSONException {
        ...
        if (isCollection) {//处理collection
            result = (T) gson.fromJson(jsonData, componentType);
        } else if (isArray) {//处理array
            result = (T) gson.fromJson(jsonData, componentType);
        } else if (isMap) {//处理Map
            result = (T) gson.fromJson(jsonData, type.getType());
            ...
            if (isPrimitiveOrWrapper(componentType)) {
                result = (T) gson.toJson(jsonData);
            } else {
                //处理自定义对象
                result = (T) gson.toJson(jsonData, componentType);
            }
        }
        return result;
    }
